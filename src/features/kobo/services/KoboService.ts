...
export class KoboService extends Effect.Service<KoboService>()("KoboService", {
	accessors: true,
	dependencies: [DatabaseLive, ConversionService.Default],
	effect: Effect.gen(function* () {
		const database = yield* DatabaseContext;

		const createKoboAuthToken = Effect.fn("KoboService.createKoboAuthToken")(
		...
		const revokeKoboAuthToken = Effect.fn("KoboService.revokeKoboAuthToken")(
			function* ({ userId, tokenId }: { userId: string; tokenId?: string }) {
		...
		const listKoboAuthTokensForUser = Effect.fn(
			"KoboService.listKoboAuthTokensForUser",
		)(function* (userId: string) {
		...
		const buildLocalLibrarySync = Effect.fn(
			"KoboService.buildLocalLibrarySync",
		)(function* ({
			userId,
			token,
			origin,
			...
			const activeBookIds = [
				...new Set(shelfBookRows.map((row) => row.bookId)),
			];
			...
			const syncedBookIds = [
				...new Set(syncedBookRows.map((row) => row.bookId)),
			];
				...
				syncScopeBookIds.length > 0
					? yield* database
							.select({
								...
								timesStartedReading:
									schema.koboReadingStates.timesStartedReading,
								...
								remainingTimeMinutes:
									schema.koboStatistics.remainingTimeMinutes,
		...
		const createMissingKepubConversionJobs = Effect.fn(
			"KoboService.createMissingKepubConversionJobs",
		)(function* (requests: KoboPendingConversion[]) {
		...
		const getBookByUuid = Effect.fn("KoboService.getBookByUuid")(function* (
			bookUuid: string,
		) {
		...
		const getBookMetadataByUuid = Effect.fn(
			"KoboService.getBookMetadataByUuid",
		)(function* ({
			bookUuid,
			origin,
			token,
		...
		const getDownloadFileForKobo = Effect.fn(
			"KoboService.getDownloadFileForKobo",
		)(function* ({
			bookId,
			requestedFormat,
		}: {
		...
		const updateReadingStateByBookUuid = Effect.fn(
			"KoboService.updateReadingStateByBookUuid",
		)(function* ({
			userId,
			bookUuid,
			payload,
			...
			yield* database
				...
				.set({
					...
					timesStartedReading:
						current.state.timesStartedReading +
						(incrementsReadingStarts ? 1 : 0),
			...
			if (statistics) {
				yield* database
					...
					.where(
						eq(schema.koboStatistics.koboReadingStateId, current.state.id),
					);
			}
				...
				UpdateResults: [
					{
						...
						PriorityTimestamp: toKoboTimestamp(
							refreshed.state.priorityTimestamp,
						),
		...
		const getReadingStateResponseByBookUuid = Effect.fn(
			"KoboService.getReadingStateResponseByBookUuid",
		)(function* ({ userId, bookUuid }: { userId: string; bookUuid: string }) {
			const data = yield* getOrCreateReadingStateByBookUuid({
				userId,
				bookUuid,
			});
			return [
		...
		const setArchivedBookByUuid = Effect.fn(
			"KoboService.setArchivedBookByUuid",
		)(function* ({
			userId,
			bookUuid,
			isArchived,
		...
		const unarchiveBooksForKoboSync = Effect.fn(
			"KoboService.unarchiveBooksForKoboSync",
		)(function* ({ userId, bookIds }: { userId: string; bookIds: string[] }) {
		...
		const createOrUpdateKoboTag = Effect.fn(
			"KoboService.createOrUpdateKoboTag",
		)(function* ({
			userId,
			name,
			revisionIds,
		...
		const renameKoboTag = Effect.fn("KoboService.renameKoboTag")(function* ({
			userId,
			tagId,
			name,
		}: {
		...
		const deleteKoboTag = Effect.fn("KoboService.deleteKoboTag")(function* ({
			userId,
			tagId,
		}: {
			userId: string;
		...
		const addItemsToKoboTag = Effect.fn("KoboService.addItemsToKoboTag")(
			function* ({
				userId,
				tagId,
				revisionIds,
		...
		const removeItemsFromKoboTag = Effect.fn(
			"KoboService.removeItemsFromKoboTag",
		)(function* ({
			userId,
			tagId,
			revisionIds,
