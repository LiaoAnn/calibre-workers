...
export class ShelfService extends Effect.Service<ShelfService>()(
	"ShelfService",
	{
		accessors: true,
		dependencies: [DatabaseLive, KoboService.Default],
		effect: Effect.gen(function* () {
			const database = yield* DatabaseContext;

			const listShelves = Effect.fn("ShelfService.listShelves")(function* (
				userId: string,
			) {
				...
				const previewRows = yield* database
					...
					.innerJoin(
						schema.books,
						eq(schema.books.id, schema.shelfBooks.bookId),
					)
					.where(inArray(schema.shelfBooks.shelfId, shelfIds))
					.orderBy(
			...
			const listShelfKoboSyncSettings = Effect.fn(
				"ShelfService.listShelfKoboSyncSettings",
			...
			const setShelfKoboSync = Effect.fn("ShelfService.setShelfKoboSync")(
				function* ({
					userId,
					shelfId,
					enabled,
					...
					if (!membership) {
						return yield* Effect.fail(
							new ShelfAccessDenied({ shelfId, userId }),
						);
					}
			...
			const listBookShelfIds = Effect.fn("ShelfService.listBookShelfIds")(
				function* ({ userId, bookId }: { userId: string; bookId: string }) {
					const rows = yield* database
			...
			const createShelf = Effect.fn("ShelfService.createShelf")(function* ({
				userId,
				name,
				visibility = "private",
			}: {
				...
				const normalizedName = yield* validateShelfName(
			...
			const updateShelf = Effect.fn("ShelfService.updateShelf")(function* ({
				userId,
				shelfId,
				name,
			}: {
			...
			const deleteShelf = Effect.fn("ShelfService.deleteShelf")(function* ({
				userId,
				shelfId,
			}: {
				userId: string;
			...
			const listShelfBooks = Effect.fn("ShelfService.listShelfBooks")(
				function* ({
					userId,
					shelfId,
					page = 1,
			...
			const addBooksToShelf = Effect.fn("ShelfService.addBooksToShelf")(
				function* ({
					userId,
					shelfId,
					bookIds,
					...
					const existingLinkSet = new Set(
						existingLinks.map((row) => row.bookId),
					);
			...
			const removeBookFromShelf = Effect.fn("ShelfService.removeBookFromShelf")(
				function* ({
					userId,
					shelfId,
					bookId,
